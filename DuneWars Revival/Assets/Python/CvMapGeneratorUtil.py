...
def addDuneFeatures():
    ...
    print "Adding Features - Dune Wars Start"
    gc = CyGlobalContext()
    iFArch   = gc.getInfoTypeForString("FEATURE_ROCK_ARCH")
    iFArch2  = gc.getInfoTypeForString("FEATURE_ROCK_ARCH2")
    iFRock1  = gc.getInfoTypeForString("FEATURE_ROCK_FORM1")
    iFRock2  = gc.getInfoTypeForString("FEATURE_ROCK_FORM2")
    iFRock3  = gc.getInfoTypeForString("FEATURE_ROCK_FORM3")
    iFRock4  = gc.getInfoTypeForString("FEATURE_ROCK_FORM4")
    iFPArch  = gc.getInfoTypeForString("FEATURE_POLAR_ROCK_ARCH")
    iFPArch2 = gc.getInfoTypeForString("FEATURE_POLAR_ROCK_ARCH2")
    iFPRock1 = gc.getInfoTypeForString("FEATURE_POLAR_ROCK_FORM1")
    iFPRock2 = gc.getInfoTypeForString("FEATURE_POLAR_ROCK_FORM2")
    iFPRock3 = gc.getInfoTypeForString("FEATURE_POLAR_ROCK_FORM3")
    iFPRock4 = gc.getInfoTypeForString("FEATURE_POLAR_ROCK_FORM4")
    iFDArch  = gc.getInfoTypeForString("FEATURE_DARK_ROCK_ARCH")
    iFDArch2 = gc.getInfoTypeForString("FEATURE_DARK_ROCK_ARCH2")
    iFDRock1 = gc.getInfoTypeForString("FEATURE_DARK_ROCK_FORM1")
    iFDRock2 = gc.getInfoTypeForString("FEATURE_DARK_ROCK_FORM2")
    iFDRock3 = gc.getInfoTypeForString("FEATURE_DARK_ROCK_FORM3")
    iFDRock4 = gc.getInfoTypeForString("FEATURE_DARK_ROCK_FORM4")
    iFCave   = gc.getInfoTypeForString("FEATURE_CAVE")
    iFPeak1  = gc.getInfoTypeForString("FEATURE_PEAK1")
    iFPeak2  = gc.getInfoTypeForString("FEATURE_PEAK2")
    iFPeak3  = gc.getInfoTypeForString("FEATURE_PEAK3")
    iTGrab   = gc.getInfoTypeForString("TERRAIN_PLAINS")
    iTPCoast = gc.getInfoTypeForString("TERRAIN_POLAR_COAST")
    iTPPlains = gc.getInfoTypeForString("TERRAIN_POLAR_PLAINS")
    iTPolar  = gc.getInfoTypeForString("TERRAIN_POLAR")
    iTCoast  = gc.getInfoTypeForString("TERRAIN_COAST")
    rockPoss = []
    polarPoss = []
    peakPoss = []
    grabPoss = []
    cavePoss = []
    rockFeaturesArray = [iFArch, iFArch2, iFRock1, iFRock2, iFRock3, iFRock4]
    polarFeaturesArray = [iFPArch, iFPArch2, iFPRock1, iFPRock2, iFPRock3, iFPRock4]
    darkFeaturesArray = [iFDArch, iFDArch2, iFDRock1, iFDRock2, iFDRock3, iFDRock4]
    peakFeaturesArray = [iFPeak1, iFPeak2, iFPeak3]
    # If any such features are on the map, don't put more
    featDict = {}
    for iFeat in rockFeaturesArray: featDict[iFeat] = 1
    for iFeat in polarFeaturesArray: featDict[iFeat] = 1
    for iFeat in darkFeaturesArray: featDict[iFeat] = 1
    for iFeat in peakFeaturesArray: featDict[iFeat] = 1
    for iPlotLoop in range(CyMap().numPlots()):
        pPlot = CyMap().plotByIndex(iPlotLoop)
        iFeat = pPlot.getFeatureType()
        if featDict.has_key(iFeat): return
    for iPlotLoop in range(CyMap().numPlots()):
        pPlot = CyMap().plotByIndex(iPlotLoop)
        iType = pPlot.getTerrainType()
        if iType == iTCoast and pPlot.getBonusType(TeamTypes.NO_TEAM) == BonusTypes.NO_BONUS:
            rockPoss.append(pPlot)
        if iType == iTGrab and pPlot.isHills() and pPlot.getBonusType(TeamTypes.NO_TEAM) == BonusTypes.NO_BONUS:
            grabPoss.append(pPlot)				
        elif (iType == iTPCoast or iType == iTPolar or iType == iTPPlains) and pPlot.getBonusType(TeamTypes.NO_TEAM) == BonusTypes.NO_BONUS:
            polarPoss.append(pPlot)	
        elif pPlot.isPeak() and pPlot.getBonusType(TeamTypes.NO_TEAM) == BonusTypes.NO_BONUS:
            peakPoss.append(pPlot)
    # Place rock outcrops on Coast
    rockCount = len(rockPoss)
    for i in range (int(rockCount / 20)):
        posn = CyGame().getSorenRandNum(rockCount, "")
        pPlot = rockPoss[posn]
        n = i % 6
        pPlot.setFeatureType(rockFeaturesArray[n], 0)
        del rockPoss[posn]
        rockCount = rockCount - 1
    # Place rock outcrops on Graben			
    grabCount = len(grabPoss)		
    for i in range (int(grabCount / 14)):
        posn = CyGame().getSorenRandNum(grabCount, "")
        pPlot = grabPoss[posn]
        n = i % 6
        pPlot.setFeatureType(darkFeaturesArray[n], 0)
        del grabPoss[posn]
        grabCount = grabCount - 1
    # Place polar rock outcrops on Polar and Polar Coast
    polarCount = len(polarPoss)		
    for i in range (int(polarCount / 8)):
        posn = CyGame().getSorenRandNum(polarCount, "")
        pPlot = polarPoss[posn]
        n = i % 6
        pPlot.setFeatureType(polarFeaturesArray[n], 0)
        del polarPoss[posn]
        polarCount = polarCount - 1
    # Place Caves and Peak features on Mesa
    cavePoss = peakPoss[:]
    caveCount = len(cavePoss)
    for i in range (int(caveCount / 2)):
        posn = CyGame().getSorenRandNum(caveCount, "")
        pPlot = cavePoss[posn]
        bSkip = false
        for dir in [W, N, E, S, NW, NE, SE, SW]:
            x, y = GetXYFromDirection(pPlot.getX(), pPlot.getY(), dir)
            if CyMap().plot(x,y).getFeatureType() == iFCave: bSkip = true
        if bSkip == true: continue
        pPlot.setFeatureType(iFCave, 0)
        del cavePoss[posn]
        caveCount = caveCount - 1
    peakCount = len(peakPoss)
    for i in range (int(peakCount / 12)):
        posn = CyGame().getSorenRandNum(peakCount, "")
        pPlot = peakPoss[posn]
        bSkip = false
        for dir in [W, N, E, S, NW, NE, SE, SW]:
            x, y = GetXYFromDirection(pPlot.getX(), pPlot.getY(), dir)
            iPlotFeat = CyMap().plot(x,y).getFeatureType()
            if iPlotFeat == iFPeak1 or iPlotFeat == iFPeak2 or iPlotFeat == iFPeak3: bSkip = true
        if bSkip == true: continue			
        n = i % 3
        pPlot.setFeatureType(peakFeaturesArray[n], 0)
        del peakPoss[posn]
