...
def adjustMap( fileName ):
	...
	# Deliverator - Start strip all Barbarian data

	newMapFile = []

	skip_rules = [
		("Platy Builder", ["Version=11"], 0),
		("ModPath=", ["EndGame"], 0),
		("AtWar=18", ["RevealMap=0"], 0),
		("TeamID=18", ["BeginPlayer"], -1),
		("Team=18", ["BeginMap"], -1),
		("UnitOwner=18", ["EndPlot", "BeginUnit", "BeginCity"], -1)
	]
	...
	for i, line in enumerate(mapFile):
		...
		if active_skip_rule == -1:
			for s, skip_rule in enumerate(skip_rules):
				if skip_rule[0] in line:
					active_skip_rule = s
					if skip_rule[2] < 0:
						newMapFile = newMapFile[:skip_rule[2]]

		else:
			skip_rule = skip_rules[active_skip_rule]
			for stop_phrase in skip_rule[1]:
				if stop_phrase in line:
					active_skip_rule = -1

		if active_skip_rule == -1:
			newMapFile.append(line)
