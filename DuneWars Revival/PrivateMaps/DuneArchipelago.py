...
# DuneWars change log: 
# 1.0.   koma13       Stub out functions to add lakes and rivers
# 1.1.   davidlallen  Postprocess to remove lakes
# 1.2.   davidlallen  Add polar ocean and coast
# 1.4.10 davidlallen  When changing terrain type, also change plot type
# 1.5.1  deliverator  Added more post-processing for new terrain layout
# 1.6    davidlallen  Remove tundra (badlands)
# 1.7    deliverator  Custom addFeatures for Dune Wars, including Caves
# 1.8    M            Some corrections and polar region tweaks
# 1.9    M            Snaky corrections  
...
def getDescription():
	return "A map consisting of many rock islands, with strips of Polar terrain in both north and south."
...
class ArchipelagoFractalWorld(CvMapGeneratorUtil.FractalWorld):
	def checkForOverrideDefaultUserInputVariances(self):
		# Set amount of Mesa here
		self.peakPercent = 200
		return
		...
		while (true):
			...
			for areaLoop in range(iNumAreas):
				...
				if pBestPlot != None:
					...
					if sPlot.getImprovementType() == gc.getInfoTypeForString("IMPROVEMENT_GOODY_HUT"): sPlot.setImprovementType(-1)
...
def isValid(playerID, x, y):
        ...
        # Deliverator disable founding on Polar
        if pPlot.getTerrainType() == gc.getInfoTypeForString("TERRAIN_POLAR"):
                return false
...
# Disable all the default operations that have no place or use in DuneWars:
def addRivers():
	return None
...
def DuneWarsPole(ix, iy, bLand):
	map = CyMap()
	pPlot = map.plot(ix, iy)
	gc = CyGlobalContext()
	pcoast = gc.getInfoTypeForString("TERRAIN_POLAR_COAST")
#!M!:	
	if not bLand and CyGame().getSorenRandNum(100, "Purge") > 25:
		pPlot.setTerrainType(pcoast, false, false)
		pPlot.setPlotType(PlotTypes.PLOT_OCEAN, false, false)
	else:
		pland = gc.getInfoTypeForString("TERRAIN_POLAR")
		pPlot.setTerrainType(pland, false, false)
		pPlot.setPlotType(PlotTypes.PLOT_LAND, false, false)
		pPlot.setBonusType(-1)
		if gc.getGame().getMapRand().get(100, "arch dw ice") < 10:
			ice = gc.getInfoTypeForString("BONUS_POLAR_ICE")
			pPlot.setBonusType(ice)
